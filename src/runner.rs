...
use crate::config::{UMI_AUTO_COVERAGE_RATIO, UmiFamilyModel};
...
/// - umi_family_model: Strategy used to define UMI families.
/// - umi_min_fraction: Minimum family fraction retained by cluster models.
/// - umi_auto_neighbor_risk: Random-neighbor threshold used by auto-model.
/// - umi_auto_coverage_ratio: Coverage-ratio threshold used by auto-model.
/// - header_match: IUPAC R1-prefix filter, if applied.
...
pub struct QuartoParams<'a> {
    ...
    strain: &'a str,
    mismatch: u8,
    umi_family_model: &'a str,
    umi_min_fraction: f64,
    umi_auto_neighbor_risk: f64,
    umi_auto_coverage_ratio: f64,
    header_match: &'a str,
    file1: &'a str,
    file2: &'a str,
...
/// * `umi_min_fraction` - Minimum family fraction retained by cluster models.
/// * `umi_auto_neighbor_risk` - Random-neighbor threshold used by auto-model.
/// * `header_match` - Optional IUPAC R1-prefix filter.
/// * `file1` - Name of the first FASTQ file (R1).
/// * `file2` - Name of the second FASTQ file (R2).
/// * `summary_data_file` - Path to the summary CSV file generated by the R script.
...
pub fn run_quarto_report(
    ...
    let umi_family_model = umi_family_model.to_string();
    let header_match = header_match.unwrap_or("not applied");
    ...
    let params = QuartoParams {
        ...
        strain,
        mismatch,
        umi_family_model: &umi_family_model,
        umi_min_fraction,
        umi_auto_neighbor_risk,
        umi_auto_coverage_ratio: UMI_AUTO_COVERAGE_RATIO,
        header_match,
        file1,
        file2,
        summary_data_file,
...
mod tests {
    ...
    #[test]
    fn test_quarto_params_include_umi_family_model() {
        let params = QuartoParams {
            version: "test-version",
            assay_type: "random-reverse",
            strain: "nl43",
            mismatch: 1,
            umi_family_model: "cluster-model",
        ...
        let json_params = serde_json::to_value(params).expect("parameters should serialize");
        assert_eq!(json_params["umi_family_model"], "cluster-model");
        assert_eq!(json_params["umi_min_fraction"], 0.005);
        assert_eq!(json_params["umi_auto_neighbor_risk"], 0.05);
        assert_eq!(json_params["umi_auto_coverage_ratio"], 0.10);
        assert_eq!(json_params["header_match"], "ARYN");
    }
    ...
    fn test_generate_html_report() {
        ...
        let mismatch = 1;
        let umi_family_model = UmiFamilyModel::MaxModel;
        let umi_min_fraction = 0.005;
        let umi_auto_neighbor_risk = 0.05;
        let header_match = None;
