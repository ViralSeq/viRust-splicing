...
fn main() {
    ...
    let weighted_seq_hash = ratio_hash.iter().map(|(k,v)| (k.clone(), (*v * number_to_generate as f32).round() as i32)).collect::<HashMap<String, i32>>();
...
fn generate_mock_sequence(
    ...
    for (splice_event,v) in weighted_seq_hash.iter() {
        ...
        let (r1_string, r2_sequence, size_tag) = from_splice_event_to_mock_r1_r2(splice_event, query_list).unwrap();
...
fn from_splice_event_to_mock_r1_r2(splice_event: &str, query_list: &HashMap<String, String>) -> Result<(String, String, String), Box<dyn Error>> {
    ...
    Ok((mock_seq_r1, reverse_complement(&mock_seq_r2.0), mock_seq_r2.1))
}
...
fn generate_r2_mock_seq() -> (String, String) {
    ...
    let selected = seq_pool.choose(&mut rand::rng())
        .unwrap()
        .clone();
