...
params:
  ...
  strain: "nl43"
  mismatch: 1
  umi_family_model: "max-model"
  umi_min_fraction: 0.005
  umi_auto_neighbor_risk: 0.05
  umi_auto_coverage_ratio: 0.10
  header_match: "not applied"
...
umi_family_model = params$umi_family_model
umi_min_fraction = params$umi_min_fraction
umi_auto_neighbor_risk = params$umi_auto_neighbor_risk
umi_auto_coverage_ratio = params$umi_auto_coverage_ratio
umi_min_fraction_display = if (umi_family_model == "max-model") "N/A" else umi_min_fraction
umi_family_mismatch_allowed = if (umi_family_model == "cluster-model") mismatch else if (umi_family_model == "cluster-model-plusone") mismatch + 1 else if (umi_family_model == "auto-model") paste0("per category: exact or ", mismatch) else "N/A"
umi_auto_settings_display = if (umi_family_model == "auto-model") paste0("max-model when unique UMI fraction is <= ", umi_auto_coverage_ratio, " or neighbor risk is >= ", umi_auto_neighbor_risk) else "N/A"
header_match = params$header_match
file1 = params$file1
file2 = params$file2
...
link_for_download <- create_embedded_csv_link(link)
...
**UMI family model:** `r umi_family_model`

**Minimum UMI family fraction:** `r umi_min_fraction_display`

**UMI family mismatch allowed:** `r umi_family_mismatch_allowed`

**UMI auto-model settings:** `r umi_auto_settings_display`

**R1 header match:** `r header_match`
...
**Output Files:**
  ...
  2. ***CSF file:*** Summarized splice isoform analysis. One record per UMI family. Families not retained by the selected UMI model are not shown.
...
<style>
  ...
  #splice_map .feat text{font-size:18px; fill:#0b1220; font-weight:600; paint-order:stroke; stroke:#ffffff; stroke-width:3px;}

  #splice_map .feat.italic text{font-style: italic;}
  ...
  #splice_map .hint .export-buttons{
    display:flex; gap:6px; flex-wrap:wrap; justify-content:center;
  }
  #splice_map .hint .export-buttons button{
...
(function(){
  ...
  // Add className per feature to style the "cell" (the <g class="feat ...">)
  // You can use a string ("italic") or an array (["italic","myTag"])
  const features = [
    {row:1,name:"gag",start:790,end:2292,color:C.gag, className:"italic"},
    {row:1,name:"vif",start:5041,end:5619,color:C.vif, className:"italic"},
    {row:1,name:"tat-2",start:8379,end:8424,color:C.tat, className:"italic"},
    {row:1,name:"nef",start:8797,end:9417,color:C.nef, className:"italic"},
    {row:2,name:"tat-1",start:5831,end:6045,color:C.tat, className:"italic"},
    {row:2,name:"vpu",start:6062,end:6310,color:C.vpu, className:"italic"},
    {row:2,name:"rev-2",start:8379,end:8653,color:C.rev, className:"italic"},
    {row:3,name:"pol",start:2085,end:5096,color:C.pol, className:"italic"},
    {row:3,name:"pro",start:2253,end:2550,color:C.pol, className:"italic"},
    {row:3,name:"vpr",start:5559,end:5850,color:C.vpr, className:"italic"},
    {row:3,name:"rev-1",start:5970,end:6045,color:C.rev, className:"italic"},
    {row:3,name:"env",start:6225,end:8795,color:C.env, className:"italic"},
  ...
  // helper: normalize className to space-prefixed string
  function classSuffix(d){
    const c=d.className;
    if(Array.isArray(c)) return c.length ? ' ' + c.filter(Boolean).join(' ') : '';
  ...
  function renderFeatures(zx){
    ...
    rowGroups.each(function(rowNum){
      ...
      const enter=sel.enter().append("g")
        .attr("class",d=>"feat"+classSuffix(d));
      ...
      // keep classes updated in case data changes
      merged.attr("class",d=>"feat"+classSuffix(d));
  ...
  // prevent page scroll over chart
  ...
  function prepareCloneForExport(svgElement) {
    const clone = svgElement.cloneNode(true);
    
    // Add background rect to SVG for consistent dark background
    const bgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
    bgRect.setAttribute("width", "100%");
    bgRect.setAttribute("height", "100%");
    bgRect.setAttribute("fill", "#2b3b4b");
    clone.insertBefore(bgRect, clone.firstChild);
    
    // Apply critical CSS styles as attributes to ensure they're preserved
    clone.querySelectorAll("path.link").forEach(el => {
      el.setAttribute("fill", "none");
      el.style.strokeOpacity = "0.35";
      el.style.pointerEvents = "stroke";
    });
    
    clone.querySelectorAll("path.connector").forEach(el => {
      // Get computed styles from original element
      const original = svgElement.querySelector("path.connector");
      if (original) {
    ...
    clone.querySelectorAll("text.arc-label").forEach(el => {
      el.style.opacity = "0";
    });
    
    clone.querySelectorAll("circle").forEach(el => {
      if (el.getAttribute("r") === "4.5") {
        el.setAttribute("fill", "#1f77b4");
        el.setAttribute("stroke", "#0b3a6f");
      }
    });
    
    clone.querySelectorAll("line.anchor-stem").forEach(el => {
      el.setAttribute("stroke", "#cdd3e1");
    });
  ...
  document.getElementById("exportSvgBtn").addEventListener("click", () => {
    const svgElement = document.getElementById("stage");
    const clone = prepareCloneForExport(svgElement);
    
    const serializer = new XMLSerializer();
    let svgString = serializer.serializeToString(clone);
    const svgWithXML = '<?xml version="1.0" encoding="UTF-8"?>\n' + svgString;
    const timestamp = new Date().toISOString().slice(0, 10);
    downloadFile(svgWithXML, `splice-map-${timestamp}.svg`, "image/svg+xml");
  });

  // Export as PNG using save-svg-as-png
  document.getElementById("exportPngBtn").addEventListener("click", () => {
    ...
    // Use save-svg-as-png library with proper background
    saveSvgAsPng(clone, `splice-map-${timestamp}.png`, {
      backgroundColor: "#2b3b4b",
      scale: window.devicePixelRatio || 1
    }).catch(err => {
      console.error("PNG export failed:", err);
      alert("Failed to export as PNG: " + err.message);
    });
  });
})();
</script>
...
[⬇️ Download this data for splice links](`r link_for_download`){download="link.csv"}
