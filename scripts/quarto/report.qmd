...
A splice event map showing the percent of reads with Dx->Ax event.
...
<style>
  ...
  #splice_map .feat text{font-size:18; fill:#0b1220; font-weight:600; paint-order:stroke; stroke:#ffffff; stroke-width:3px}
  #splice_map .link{fill:none;stroke-opacity:.35;pointer-events:stroke}
  #splice_map .link:hover{stroke-opacity:.9}

  /* Hide arc labels until hover; we'll toggle via JS .style('opacity', ...) */
  ...
  /* Connector arcs between features */
  ...
  #splice_map .hint{
    ...
    padding:6px 10px; font:12px/1.1 system-ui,-apple-system,Segoe UI,Roboto,sans-serif;
    display:flex; gap:8px; align-items:center; user-select:none; pointer-events:none;
  }
...
(function(){
  ...
  const anchors = [
    ...
    {id:"A4",pos:5955},{id:"A5",pos:5977},{id:"A7",pos:8379}
  ];
  ...
  function drawAxis(zx){
    xAxisG.call(d3.axisBottom(zx).tickValues(tickVals).tickFormat(d3.format("~d")).tickSizeOuter(0));
  }
  drawAxis(x);
  ...
  function bridgePath(x1,y1,x2,y2,k=0.22){
    ...
    const lift = Math.max(16, span*k) * -0.35; // arc upward
  ...
  function renderConnectors(zx){
    const data=connectors.map(c=>{
      const f=features.find(d=>d.name===c.from);
      const t=features.find(d=>d.name===c.to);
      if(!f||!t) return null;
      return {
        path: bridgePath(
          zx(f.end), rowBand(f.row)+rowBand.bandwidth()/2,
          zx(t.start), rowBand(t.row)+rowBand.bandwidth()/2
        )
      };
    }).filter(Boolean);

    const sel=connG.selectAll("path.connector").data(data);
    sel.enter().append("path").attr("class","connector")
      .merge(sel)
      .attr("d",d=>d.path);
    sel.exit().remove();
  }
  ...
  function layoutAnchorLabels(zx){
    const MIN_DX=26,STEP=12;
    let lastX=-Infinity,level=0;
    anchorNodes.select("text").attr("y",(d)=>{
      const xpx=zx(d.pos);
      if(xpx-lastX<MIN_DX) level++; else level=0;
      lastX=xpx;
      return -14-level*STEP;
    });
    anchorNodes.select("line.anchor-stem")
      .attr("y2",function(){
        const ty=+d3.select(this.parentNode).select("text").attr("y");
        return ty+4;
      });
  }
  layoutAnchorLabels(x);

  // ========= FLOW ARCS + HOVER LABELS ======================================
  ...
  function renderLinks(zx){
    ...
    const pathEnter=pathSel.enter().append("path").attr("class","link")
      ...
      .on("mouseenter", function(event,d){
        ...
        labelG.select(`#lbl-${d.uid}`).style("opacity", 1);   // <-- style(), not attr()
      })
      .on("mouseleave", function(event,d){
        d3.select(this).style("stroke-opacity", 0.35);
        labelG.select(`#lbl-${d.uid}`).style("opacity", 0);   // <-- style(), not attr()
      });
    ...
    // define label paths
    links.forEach((d)=>{
      const pid=`arc-label-path-${d.uid}`;
    ...
    // labels (hidden by default; shown on hover via .style)
    const lblSel=labelG.selectAll("text.arc-label").data(links,d=>d.uid);
    const lblEnter=lblSel.enter().append("text")
      .attr("class","arc-label")
      .attr("dy",14)
      .attr("id",d=>`lbl-${d.uid}`)   // targeted on hover
      .style("opacity", 0);           // explicit initial style to be safe

    lblEnter.append("textPath")
      .attr("href",d=>`#arc-label-path-${d.uid}`)
      .attr("startOffset","50%")
      .attr("text-anchor","middle");
