...
```{=html}
<!-- Arc-style chord diagram with HXB2 gene map aligned to x-scale -->
<!-- Arc-style chord diagram with HXB2 gene map aligned to the x-scale -->
<style>
  #splice-map .wrap{padding:16px}
  #splice-map svg{width:100%;height:620px;background:#f6f9fc;}
  #splice-map .axis line,#splice-map .axis path{stroke:#555}
  #splice-map .tick text{fill:#333;font-size:12px}
  #splice-map .anchor text{font-size:12px;dominant-baseline:hanging}
  #splice-map .link{fill:none;stroke-opacity:.25;pointer-events:stroke}
  #splice-map .link:hover{stroke-opacity:.9}
  #splice-map .arc-label{
  ...
  #splice-map, #splice-map .wrap, #splice-map svg { overscroll-behavior: contain; }
  #splice-map .wrap { overflow: hidden; }
</style>
...
window.addEventListener('DOMContentLoaded', () => {
  const root = d3.select("#splice-map");
  const wrap = root.select(".wrap").node();
  const W = Math.max(800, wrap.clientWidth),
        H = 560,
        M = {t:40,r:24,b:60,l:24},
        innerW = W - M.l - M.r,
        innerH = H - M.t - M.b,
        baselineY = innerH / 2;
  ...
  const cfg = { colors:{D1:"#4a90e2",D2:"#f5a623",D3:"#999",D4:"#777"} };
  const dir = () => "down"; // force all arcs downward
  ...
  if (!links.length) {
    ...
    svg.append("text")
      ...
      .attr("dominant-baseline","middle").attr("fill","#777").attr("font-size",20)
      .text("No links found");
    return;
  }

  // stroke width from percent (perceptual)
  const maxPercent = d3.max(links, d => d.percent);
  const widthScale = d3.scaleSqrt().domain([0, maxPercent]).range([1, 15]);
  links = links.map(l => ({
    ...l,
    label: `${l.from}➞${l.to} ${l.percent}%`,
    strokeWidth: widthScale(l.percent)
  }));

  // --- Setup ---
  const svg = root.select("#c")
    .attr("viewBox",`0 0 ${W} ${H}`)
    .attr("preserveAspectRatio","xMidYMid meet");
  const g = svg.append("g").attr("transform",`translate(${M.l},${M.t})`);

  // ---- Piecewise x-scale with stretch on [4500, 6000]; full HXB2 to 9719 ----
  const genomeMax = 9719;
  const focusStart = 4500, focusEnd = 6000, stretch = 2;
  const L1 = focusStart, L2 = focusEnd - focusStart, L3 = genomeMax - focusEnd;
  const totalWeighted = L1 + L2 * stretch + L3;
  const unit = innerW / totalWeighted;
  const r1 = L1 * unit, r2 = r1 + (L2 * stretch) * unit, r3 = r2 + L3 * unit;

  const x = d3.scaleLinear()
    .domain([0, focusStart, focusEnd, genomeMax])
    .range([0, r1, r2, r3]);

  // Axis in increments of 100
  const tickVals = d3.range(0, genomeMax + 1, 1000);
  ...
  axisG.append("line")
    ...
    .attr("stroke","#333").attr("stroke-width",1.25);

  const grid = axisG.selectAll(".v")
    .data(tickVals.filter(d => d % 1000 === 0)) // light verticals each 1000
    .join("line")
      .attr("class","v")
      .attr("x1",d=>x(d)).attr("x2",d=>x(d))
      .attr("y1",-innerH/2).attr("y2",innerH/2)
      .attr("stroke","#fff").attr("stroke-width",1).attr("opacity",.6);
  ...
  g.append("text")
    .attr("x",innerW/2).attr("y",H - M.t - 6)
    .attr("text-anchor","middle").attr("fill","#333")
    .text("HXB2 genome position (nt)");

  // --- Gene-map image with left/right margin calibration ---
  const imgURL = "https://www.hiv.lanl.gov/content/sequence/HIV/IMAGES/gene-map-hxb2-1200px.jpg";
  const baseImgHeight = 110;
  const imgY0 = baselineY - baseImgHeight - 10;

  // Fractions of image width where genome 0 and 9719 land.
  // Tweak u0/u1 slightly if you notice misalignment.
  const imgCal = { u0: 0.07, u1: 0.97 };

  // initial calibrated placement
  const imgWidth0 = (x(genomeMax) - x(0)) / (imgCal.u1 - imgCal.u0);
  const imgX0     = x(0) - imgCal.u0 * imgWidth0;

  const img = g.append("image")
    .attr("href", imgURL)
    .attr("x", imgX0)
    .attr("y", imgY0)
    .attr("width", imgWidth0)
    .attr("height", baseImgHeight)
    .attr("preserveAspectRatio", "none")
    .style("pointer-events","none");

  // --- Anchors (for dots & ids) ---
  const A = Object.fromEntries(anchors.map(a=>[a.id,a]));
  const anchorNodes = g.append("g").attr("class","anchor").selectAll("g")
    .data(anchors).join("g")
  ...
  anchorNodes.append("text")
    .attr("y",12).attr("text-anchor","middle").attr("fill","#333").text(d=>d.id);

  // --- Arc path (down) ---
  const arcPath = (x0,x1,_="down",k=.33) => {
    const mid=(x0+x1)/2, s=1, h=Math.max(40, Math.abs(x1-x0)*k);
    return `M${x0},${baselineY} Q${mid},${baselineY + s*h} ${x1},${baselineY}`;
  };
  ...
  linkG.selectAll("path")
    ...
    .join("path")
      .attr("class","link")
      .attr("id", (_,i)=>`arc-${i}`)
      .attr("stroke", d=>cfg.colors[d.from] || "#888")
      .attr("stroke-width", d=>d.strokeWidth)
      .attr("d", d=>{
        const x0=x(A[d.from].pos), x1=x(A[d.to].pos);
        const j=(Math.random()*0.08)-0.04;
        return arcPath(x0,x1,"down",0.33+j);
      });
  ...
  const zoomed = e => {
    const zx = e.transform.rescaleX(x);
    ...
    grid.attr("x1",d=>zx(d)).attr("x2",d=>zx(d));
    ...
    // links
    g.selectAll(".link")
      .attr("d", d => arcPath(zx(A[d.from].pos), zx(A[d.to].pos), "down", .33));

    // image: keep calibrated and scale height with zoom
    const wScaled = (zx(genomeMax) - zx(0)) / (imgCal.u1 - imgCal.u0);
    const xScaled = zx(0) - imgCal.u0 * wScaled;
    const newH = baseImgHeight * k;

    img
      .attr("x", xScaled)
      .attr("width", wScaled)
      .attr("y", imgY0 - (newH - baseImgHeight)) // grow upward
      .attr("height", newH);
  };
