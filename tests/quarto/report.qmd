...
<style>
  ...
  #splice_map .arc-label{font:18px/1.2 system-ui,-apple-system,Segoe UI,Roboto,sans-serif; fill:#0b1220; paint-order:stroke; stroke:#ffffff; stroke-width:3px; pointer-events:none;}
...
(function(){
  ...
  const rows = Array.from(new Set(features.map(d => +d.row))).sort((a,b)=>a-b);
  const trackHeightPortion = 0.6; // top portion for rows
  const rowBand = d3.scaleBand()
    .domain(rows)
    .range([0, innerH * trackHeightPortion])
    .paddingInner(0.28)
    .paddingOuter(0.1);
  ...
  // Axis & grid
  let tickStep = 1000;
  let tickVals = d3.range(0, GENOME_MAX + 1, tickStep);
  if (!tickVals.includes(GENOME_MAX)) tickVals.push(GENOME_MAX); // ensure end tick
