...
```{=html}
<!-- Arc-style chord diagram (static labels, scoped safely) -->
<style>
  ...
  #splice-map .highlight{stroke-opacity:.95!important}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Data ---
  const anchors = [
    {id:"D1",pos:900},{id:"D2",pos:5100},{id:"D3", pos:5500},{id:"A1",pos:4800},{id:"A2",pos:5400},
    {id:"A3",pos:5700},{id:"A4",pos:5900},{id:"A5",pos:6000},{id:"A7",pos:8200}
  ];
  const cfg = {
    colors:{D1:"#4a90e2",D2:"#f5a623",D3:"#999",D4:"#777"},
    directionsUp:["D1","A2"]
  };
  const dir = f => cfg.directionsUp.includes(f) ? "up" : "down";
  ...
  // >>>>>>> PIECEWISE SCALE WITH STRETCH ON [4500, 6000] <<<<<<<
  const focusStart = 4500, focusEnd = 6000, stretch = 2; // make middle 2× wider
  const L1 = focusStart - 0;         // 0..4500
  const L2 = focusEnd - focusStart;  // 4500..6000
  const L3 = 9000 - focusEnd;        // 6000..9000

  const W1 = L1;           // normal
  const W2 = L2 * stretch; // stretched
  const W3 = L3;           // normal

  const totalWeighted = W1 + W2 + W3;
  const unit = innerW / totalWeighted;

  const r0 = 0;
  const r1 = r0 + W1 * unit;        // mapped position for 4500
  const r2 = r1 + W2 * unit;        // mapped position for 6000
  const r3 = r2 + W3 * unit;        // mapped position for 9000 (≈ innerW)

  // Piecewise linear scale: domain breakpoints map to range breakpoints.
  const x = d3.scaleLinear()
    .domain([0, focusStart, focusEnd, 9000])
    .range([0, r1, r2, r3]);
  ...
  // --- Arc path generator ---
  const arcPath = (x0,x1,d="up",k=.33) => {
    const mid=(x0+x1)/2, s = d==="down"?1:-1, h = Math.max(40, Math.abs(x1-x0)*k);
  ...
  linkG.selectAll("path")
    ...
    .join("path")
      ...
      .attr("d", d=>{
        ...
        return arcPath(x0,x1,dir(d.from),0.33+j);
      })
    .append("title")
      .text(d=>`${d.label} (w=${d.w}${d.percent!=null?`, ${d.percent}%`:''})`);
  ...
  // --- Legend ---
  const legend = svg.append("g").attr("transform",`translate(${W-190},${M.t})`);
  [...new Set(links.map(d=>d.from))].forEach((k,i)=>{
    const lg = legend.append("g").attr("transform",`translate(0,${i*20})`);
    lg.append("line").attr("x1",0).attr("x2",20).attr("y1",8).attr("y2",8)
      .attr("stroke",cfg.colors[k]||"#888").attr("stroke-width",6);
    lg.append("text").attr("x",28).attr("y",12).attr("fill","#333").text(k);
  });

  // --- Zoom & Pan (x-axis only) ---
  const zoomed = e => {
    const zx = e.transform.rescaleX(x);
    g.select(".axis").call(xAxis.scale(zx));
    anchorNodes.attr("transform", d => `translate(${zx(d.pos)},${baselineY})`);
    g.selectAll(".link")
      .attr("d", d => arcPath(zx(A[d.from].pos), zx(A[d.to].pos), dir(d.from), .33));
    axisG.selectAll(".v").attr("x1",d=>zx(d)).attr("x2",d=>zx(d));
  };
  svg.call(
    d3.zoom()
      .scaleExtent([1,20])
