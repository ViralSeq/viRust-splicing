...
<style>
  ...
  #splice_map .arc-label{font:16px/1.2 system-ui,-apple-system,Segoe UI,Roboto,sans-serif; fill:#0b1220; paint-order:stroke; stroke:#ffffff; stroke-width:3px; pointer-events:none;}
  #splice_map .board, #splice_map svg { overscroll-behavior: contain; }

  /* NEW: anchor labels above axis */
...
(function(){
  ...
  const arcAreaBottom=innerH;

  // Axis
  const tickStep=1000;
  const tickVals=d3.range(0,GENOME_MAX+1,tickStep);
  ...
  // ========= FEATURE CONNECTIONS ============================================
  // Connect tat-1→tat-2 and rev-1→rev-2 with simple lines
  const connectors=[
    {from:"tat-1",to:"tat-2"},
    {from:"rev-1",to:"rev-2"}
  ];
  ...
  function renderConnectors(zx){
    const data=connectors.map(c=>{
      ...
      if(!f||!t) return null;
      return {
        x1:zx(f.end), y1:rowBand(f.row)+rowBand.bandwidth()/2,
        x2:zx(t.start), y2:rowBand(t.row)+rowBand.bandwidth()/2
      };
    }).filter(Boolean);
    const sel=connG.selectAll("line").data(data);
    sel.enter().append("line").attr("stroke","#ffffff55").attr("stroke-width",2)
      .merge(sel)
  ...
  function renderLinks(zx){
    ...
    pathSel.enter().append("path").attr("class","link")
